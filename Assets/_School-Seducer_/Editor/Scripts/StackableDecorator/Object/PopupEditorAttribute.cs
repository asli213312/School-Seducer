            ...
            public static void Show(Rect rect, Object obj, float width)
            {
                ...
                if (EditorUtility.IsPersistent(obj))
                {
                    ...
                    if (popup.m_AssetEditor != null)
                    {
                        PropertyInfo prop;
#if UNITY_2017_1_OR_NEWER
                        if (popup.m_AssetEditor is UnityEditor.AssetImporters.AssetImporterEditor)
                        {
                            prop = typeof(UnityEditor.AssetImporters.AssetImporterEditor).GetProperty("assetEditor", BindingFlags.Instance | BindingFlags.NonPublic);
                            prop.SetValue(popup.m_AssetEditor, popup.m_Editor, null);
                            popup.m_ShowImportedObject = (popup.m_AssetEditor as UnityEditor.AssetImporters.AssetImporterEditor).showImportedObject;
                        }
                        else
