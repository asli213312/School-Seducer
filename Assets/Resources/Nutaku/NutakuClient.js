var Nutaku = 
{
	...
	createItem : function(data)
	{
		...
		itemParams[opensocial.BillingItem.Field.SKU_ID] = data.id;
		itemParams[opensocial.BillingItem.Field.PRICE] = data.price * data.count;
		itemParams[opensocial.BillingItem.Field.COUNT] = 1;
		itemParams[opensocial.BillingItem.Field.DESCRIPTION] = data.description;
		itemParams[nutaku.BillingItem.Field.NAME] = data.name;
	...
	createPayment : function(items, description)
	{
		...
		params['playFabId'] = 'BFB68AEA557DB620';
		params['sessionTicket'] = 'BFB68AEA557DB620-F3C51A68E1305C6D-149F97F503108486-98D64-8DC50DB2A7FB0D2-LjjjkeNglZ7bZjdyxQ3JsfxbL0+bNIDjR8NGdf0G+JU=';
	...
	requestPayment : function(dataPayment, callback)
	{
		console.log("Nutaku::requestPayment");
		
		data = JSON.parse(dataPayment);
		console.log(data);
		...
		if (description == null || description == "") description = "store desc";
		...
		var payment = this.createPayment(paymentItems, description);
